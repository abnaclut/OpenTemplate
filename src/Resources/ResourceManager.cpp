...
{
    if (const auto it = m_sprites.find(spriteName); it != m_sprites.end()) { return it->second; }
    //else
    std::cerr << "Sprite not found: " << spriteName << "\n";
    return nullptr;
}

void ResourceManager::setExecutablePath(const std::string& executablePath) { m_path = executablePath; }
...
void ResourceManager::unloadResource(const std::string& resourceName) { //FIXME: implement me
}
